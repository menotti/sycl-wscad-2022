...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        for (auto it = 0u; it < inter; it++) {
            ...
            auto num_groups = sycl::range<2>(dimx/grp_size, dimy/grp_size);
            auto group_size = sycl::range<2>(grp_size, grp_size);            
            ...
            q.submit([&](sycl::handler &h) {
                ...
                h.parallel_for_work_group(num_groups, group_size, [=](sycl::group<2> grp) {
                    ...
                    grp.parallel_for_work_item([&](sycl::h_item<2> it) {
                        //calculus to obtain global position 
                        int x = jb * grp_size + it.get_local_id(0);
                        int y = ib * grp_size + it.get_local_id(1);
                        int pixel_x, pixel_y;
                        ...
                        int val = (int) std::sqrt(pixel_x*pixel_x+ pixel_y*pixel_y);
                        if(val < 0) val = 0; 
                        if(val > 255) val = 255;
